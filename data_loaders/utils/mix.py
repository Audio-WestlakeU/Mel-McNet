...
def pad_or_cut(wavs: List[ndarray], lens: List[int], rng: Generator) -> List[ndarray]:
    ...
    for i, wav in enumerate(wavs):
        if wav.ndim == 1:
            ...
            # repeat
            while wav.shape[0] < lens[i]:
                wav = np.concatenate([wav, wav], axis=0)
            # cut to needed length
            if wav.shape[0] > lens[i]:
                start = rng.integers(low=0, high=wav.shape[0] - lens[i] + 1)
                wav = wav[start:start + lens[i], :]    
        else:
            raise ValueError(f"wavs should have ndim 1 or 2, but got {wav.ndim}")
        wavs[i] = wav
    return wavs
