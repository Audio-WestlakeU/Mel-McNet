...
class ISTFT(nn.Module):
    ...
    def forward(self, spec: torch.Tensor, original_len: int = None) -> torch.Tensor:
        """
        ...
        if self.padding == "center":
            ...
            return torch.istft(spec, self.n_fft, self.hop_length, self.win_length, self.window, center=True, length=original_len)
...
class ISTFTHead(FourierHead):
    ...
    def forward(self, x: torch.Tensor, original_len: int = None) -> torch.Tensor:
        """
        ...
        S = mag * (x + 1j * y)
        audio = self.istft(S, original_len)
        # return audio
        return audio

class VOCOS_DECODE(nn.Module):
    """
    ...
    def __init__(
        self,
        backbone: nn.Module = VocosBackbone,
        head: nn.Module = ISTFTHead,
    ) :
        super().__init__()
        self.backbone = backbone
        self.head = head

    def forward(self, features, original_len: int = None, **kwargs) -> torch.Tensor:
        x = self.backbone(features, **kwargs)
        audio_output = self.head(x, original_len)
        return audio_output