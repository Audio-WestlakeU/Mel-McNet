...
def on_load_checkpoint(
    ...
    if reset is not None:
        for key in reset:
            assert key in ['optimizer',
                           'lr_scheduler'], f'unsupported reset key {key}'
            if key == 'optimizer':
...
def test_setp_write_example(self, xr: Tensor, yr: Tensor, yr_hat: Tensor, sample_rate: int, paras: Dict[str, Any], result_dict: Dict[str, Any], wavname: str, exp_save_path: str):
    ...
    elif paras[0]['dataset'] == 'realman':
        wav_path = os.path.join(exp_save_path, 'examples',wavname)
