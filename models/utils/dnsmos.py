...
def deep_noise_suppression_mean_opinion_score_new(preds: Tensor, fs: int, personalized: bool, device: Optional[str] = None) -> Tensor:
    """Calculate `Deep Noise Suppression performance evaluation based on Mean Opinion Score`_ (DNSMOS).
    ...
    for idx in range(num_hops):
        ...
        p808_oi = {"input_1": p808_input_features}
        mos_np = np.concatenate([p808_onnx_sess.run(None, p808_oi)[0], onnx_sess.run(None, oi)[0]], axis=-1, dtype="float64")
        mos_np_raw = copy.deepcopy(mos_np).reshape(shape[:-1] + (4, ))
        moss_raw.append(mos_np_raw)
        mos_np = _polyfit_val(mos_np, personalized)

        mos_np = mos_np.reshape(shape[:-1] + (4, ))
        moss.append(mos_np)
    return torch.from_numpy(np.mean(np.stack(moss, axis=-1), axis=-1)), torch.from_numpy(np.mean(np.stack(moss_raw, axis=-1), axis=-1))  # [p808_mos, mos_sig, mos_bak, mos_ovr], [p808_mos_raw, mos_sig_raw, mos_bak_raw, mos_ovr_raw]